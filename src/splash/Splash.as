package splash 
{
	...
	public class Splash extends Entity
	{
		...
		/**
		 * Starting, Stopping, isPlaying information.
		 */
		public var isLogoFinished:Boolean = false; 
		...
		{	
			...
			FP.alarm(_fadeTime, function ():void {	// wait a bit before animating the cogs
				cogs.play("shoot", true);
			});
			FP.alarm(_fadeTime, function ():void {	// wait a bit before animating the text
			...
			FP.alarm(_logoTime + _fadeTime, function ():void {
		...
		{	
			...
			FP.log("Logo is finished");
			if (_onComplete == null) {
				FP.log("callBack is null");
				FP.world.remove(this);
				Assets.gcHackLog(0.25);
				return;
			}
			else if (_onComplete is Function) {
				FP.log("callBack is Function");
				_onComplete();
				FP.world.remove(this);
				Assets.gcHackLog(0.25);
				FP.log("Click to activate");
			}
			else if (_onComplete is World) {
				FP.log("callBack is World");
				FP.world = _onComplete;
				FP.world.remove(this);
				Assets.gcHackLog(0.25);
				FP.log("Click to activate");
			}
